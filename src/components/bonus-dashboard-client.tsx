...
import { getActivePerformanceData, getMonthlyRepresentatives, getPerformanceMonthsByImportRecency, getPerformanceShopActuals, getShopMetrics, type BonusSnapshot, type PerformanceMetric, type Target } from "@/lib/types";
...
export function BonusDashboardClient() {
  ...
  const representatives = latestImport
    ...
    : getMonthlyRepresentatives(selectedShop, selectedMonth);
  ...
  return <div className="flex h-full flex-col"><Header title={`${t("bonusPage")}: ${selectedShop.name}`} actions={<><Link href={`/${locale}/`} className={buttonVariants({ variant: "outline", size: "sm" })}><ArrowLeft className="mr-1.5 h-4 w-4" />{t("backToOverview")}</Link><ShopPageNav shopId={selectedShop.id} active="bonus" /><select aria-label="Import month" className="h-9 w-40 shrink-0 rounded-md border bg-background px-3 text-sm text-foreground" value={selectedMonth} onChange={event => setMonthSelection({ shopId: selectedShop.id, month: event.target.value })}>{months.map(month => <option key={month} value={month}>{format(parseISO(`${month}-01`), "MMMM yyyy")}</option>)}</select></>} /><div className="flex-1 overflow-y-auto p-3 md:p-4"><div className="mx-auto w-full max-w-6xl space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-semibold">{t("monthlyBonuses")}</h2><p className="text-sm text-muted-foreground">{selectedMonth ? t("bonusMonthDescription", { month: format(parseISO(`${selectedMonth}-01`), "MMMM yyyy") }) : t("noData")}</p></div><Button onClick={finalize} disabled={!liveManager || !!snapshot || finalizing}><CalendarCheck className="mr-2 h-4 w-4" />{snapshot ? "Finalized" : finalizing ? "Finalizing…" : "Finalize month"}</Button></div>{snapshot && <p className="flex items-center gap-2 rounded-md border border-emerald-500/40 bg-emerald-500/10 p-3 text-sm text-emerald-700 dark:border-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300"><CheckCircle2 className="h-4 w-4" />Payroll snapshot finalized {new Date(snapshot.finalizedAt).toLocaleString(locale)}. Displaying locked values.</p>}<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{cards.map(item => <Card key={item.label}><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">{item.label}</CardTitle><item.icon className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><p className="text-xl font-bold">{item.value}</p></CardContent></Card>)}</div>{collection !== undefined && managerResult ? <Tabs defaultValue="manager"><TabsList className="grid w-full grid-cols-2 sm:w-[420px]"><TabsTrigger value="manager"><BriefcaseBusiness className="mr-2 h-4 w-4" />{t("managerBonusButton")}</TabsTrigger><TabsTrigger value="representatives"><Users className="mr-2 h-4 w-4" />{t("representativeBonusButton")}</TabsTrigger></TabsList><TabsContent value="manager" className="mt-4"><ManagerBonusCard monthlyCollection={collection} actuals={totals.shop} targets={targets} metrics={metrics} metricSettings={metricSettings} result={managerResult} /></TabsContent><TabsContent value="representatives" className="mt-4"><RepresentativeBonusCards representatives={displayRepresentatives} individualActuals={totals.reps} individualTargets={individualTargets} shopActuals={totals.shop} shopTargets={targets} metrics={metrics} monthlyCollection={collection} metricSettings={metricSettings} savedResults={Object.fromEntries(representativeResults.map(item => [item.id, item.result]))} /></TabsContent></Tabs> : <p className="rounded-lg border p-6 text-sm text-muted-foreground">{t("managerBonusMissingCollection")}</p>}</div></div></div>;
}
